...
public:
	...
	*/
	VkResult acquireNextImage(VkSemaphore presentCompleteSemaphore, uint32_t *imageIndex)
	{
		...
		VkResult res = fpAcquireNextImageKHR(device, swapChain, UINT64_MAX, presentCompleteSemaphore, (VkFence)nullptr, imageIndex);
		assert(VK_SUCCESS == res && "vkAcquireNextImageKHR failed");
		return res;
	}
	...
	*/
	VkResult queuePresent(VkQueue queue, uint32_t imageIndex, VkSemaphore waitSemaphore = VK_NULL_HANDLE)
	{
		...
		VkResult res = fpQueuePresentKHR(queue, &presentInfo);
		assert(VK_SUCCESS == res && "vkQueuePresentKHR failed");
		return res;
	}
